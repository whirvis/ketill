 ...
 * @see #getAdapters()
 */
public class USBGameCubeDevice implements UsbPipeListener {
	...
	private final USBGameCubeControllerAdapter[] adapters;
	private final List<USBGameCubeControllerAdapter> adapterList;
	 ...
	 */
	public USBGameCubeDevice(UsbDevice device) {
		...
		this.adapters = new USBGameCubeControllerAdapter[4];
		for (int i = 0; i < adapters.length; i++) {
			this.adapters[i] = new USBGameCubeControllerAdapter(this, i);
		}
		this.adapterList =
	...
	public void dataEventOccurred(UsbPipeDataEvent event) {
		...
		if (id == DATA_ID) {
			...
			for (USBGameCubeControllerAdapter adapter : adapters) {
				adapter.poll();
			}
