...
abstract class HidPs4Adapter extends IoDeviceAdapter<Ps4Controller> {
    ...
    @FeatureAdapter
    void updateMotor(@NotNull MotorVibration state, int byteOffset) {
        byte forceByte = (byte) (state.getStrength() * 0xFF);
        this.outputReport[byteOffset] = forceByte;
    }

    @FeatureAdapter
    void updateLightbar(@NotNull LightbarColor state, int byteOffset) {
        Vector4fc vector = state.getVector();
