...
public final class LibUsbPs3Adapter extends IoDeviceAdapter<Ps3Controller> {
    ...
    @FeatureAdapter
    private void updateWeakMotor(@NotNull MotorVibration state,
                                 int byteOffset) {
        byte rumbleWeak = (byte) (state.getStrength() > 0.0F ? 0x01 : 0x00);
    ...
    @FeatureAdapter
    private void updateStrongMotor(@NotNull MotorVibration state,
                                   int byteOffset) {
        byte rumbleStrong = (byte) (state.getStrength() * 255.0F);
    ...
    @FeatureAdapter
    private void updateLed(@NotNull LedState state, int byteOffset) {
        ...
        } else if (mode == LedState.MODE_BITWISE) {
            ledByte = (byte) state.getValue();
        }
